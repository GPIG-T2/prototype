...
class Model:
    ...
    async def populate_initial_data(self) -> None:
        ...
        self.layout = await self._client.get_layout()

        # Get all the areas
        areas = await self._client.get_areas(self.layout.areas)
    ...
    async def load_area_information(self, area: Area):
        ...
        transports = await self._client.get_transport_links(transport_link_ids_to_request)

        transport: TransportLink
        for transport in transports:
            self.transport_links[transport.id] = transport
        ...
        organisations = await self._client.get_organisations(organisation_ids_to_request)
